...
from videomind.constants import GROUNDER_PROMPT, CHECKER_PROMPT
...
if __name__ == '__main__':
    ...
    # NOTE: grounder is always true so no need to store
    adapter_state = dict(checker=False)

    print('Initializing role *grounder*')
    model, processor = build_model(args.model_gnd_path, device=args.device)
    device = next(model.parameters()).device

    if args.model_chk_path is not None:
        adapter_path = nncore.join(args.model_chk_path, 'checker')
        if nncore.is_dir(adapter_path):
            print('Initializing role *checker*')
            model.load_adapter(adapter_path, adapter_name='checker')
            adapter_state['checker'] = True
    ...
    for i in nncore.ProgressBar(range(len(annos))):
        ...
        question = anno['question'] if 'question' in anno else anno['query']
        print(question)
            ...
            for cand in pred[:5]:
                ...
                messages = [{
                    ...
                    'content': [{
                        ...
                        'type': 'text',
                        'text': CHECKER_PROMPT.format(question)
                    }]
                }]
